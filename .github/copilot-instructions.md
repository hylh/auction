...
Provide deterministic seed data for demo users, fish items, auctions, bids, and sales.

Provide an optional simulator command that creates auctions and bids through the same server/API path used by the application. The simulator should exercise validation, persistence, metrics, and real-time updates rather than inserting directly into the database.

The simulator should be configurable with options such as auction count, bid interval, and duration.

## Observability

Include application and domain observability from the start.
...
- Auctions closed.
- Sales completed.
- Total sale value in cents.
- Request latency.
- Bid mutation duration.

## Testing expectations

Use Vitest for:

- Unit tests around domain rules, especially money, weight conversion, bid increments, stale bid rejection, auction state transitions, and permission rules.
- Integration tests around server functions and database behavior.
- Server-level real-time tests for the SSE/event broadcaster, including multiple listeners receiving the same ordered bid event after a bid is accepted.

Do not add browser automation or Playwright unless explicitly requested later.

## Implementation priorities

Start with a working vertical slice before adding polish:

1. Scaffold TanStack Start with pnpm and required tooling.
2. Add PostgreSQL, Drizzle schema, migrations, and seed data.
3. Implement fish inventory, auctions, bids, and sales domain logic.
4. Implement live auction detail with transactional bidding and SSE updates.
5. Implement the live dashboard.
6. Implement the history/admin/statistics dashboard.
7. Add metrics, structured logs, and OpenTelemetry instrumentation.
8. Add simulator commands.
9. Expand Vitest unit and integration coverage.

Prefer correctness, type safety, and clear domain boundaries over shortcuts. Keep the application demonstrable locally and easy for another TypeScript developer to understand.
