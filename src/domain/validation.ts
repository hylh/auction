...
const integerNumberSchema = (schema: z.ZodNumber) =>
  z.preprocess((value) => {
    if (typeof value !== "string") {
      return value;
    }
...
export const fishInputSchema = z.object({
  ...
  startingPriceMajor: integerNumberSchema(z.number().int().positive().max(10_000_000)),
  sellerId: uuidSchema,
...
export const simulatorInputSchema = z.object({
  ...
  bidCount: z.coerce.number().int().min(0).max(50).default(1),
  bidRounds: z.coerce.number().int().min(0).max(20).optional(),
  ...
  auctionIds: z.array(uuidSchema).max(10).optional(),
